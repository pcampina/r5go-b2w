...
@section('content')
    <div class="page-content container-fluid">
        <form class="form-edit-add" role="form" action="@if($edit){{ route('voyager.products.update', $dataTypeContent->id) }}@else{{ route('voyager.products.store') }}@endif" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-md-8">
                    ...
                    <div class="panel">
                        ...
                        <div class="panel-heading">
                            <h3 class="panel-title">
                                <i class="voyager-character"></i> {{ __('Product') }}
                                <span class="panel-desc"> {{ __('Product_sub') }}</span>
                            </h3>
                    ...
                    <div class="panel">
                        <div class="panel-heading">
                            <h3 class="panel-title">{{ __('Content') }}</h3>
                            <div class="panel-actions">
                    ...
                    <div class="panel">
                        <div class="panel-heading">
                            <h3 class="panel-title">{{ __('Additional Fields') }}</h3>
                            <div class="panel-actions">
                                <a class="panel-action voyager-angle-down" data-toggle="panel-collapse" aria-hidden="true"></a>
                            </div>
                        </div>
            ...
            @section('submit-buttons')
                <button type="submit" class="btn btn-primary pull-right">
                     @if($edit){{ __('Update') }}@else <i class="icon wb-plus-circle"></i> {{ __('New') }} @endif
                </button>
            @stop
        ...
        <form id="my_form" action="{{ route('voyager.upload') }}" target="form_target" method="product"
    ...
    <script>
        ...
        $('document').ready(function () {
            ...
            $('#confirm_delete').on('click', function(){
                $.product('{{ route('voyager.'.$dataType->slug.'.media.remove') }}', params, function (response) {
                    if ( response
                        && response.data
